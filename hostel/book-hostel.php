...
{
	$stmt=$mysqli->prepare("SELECT regno FROM hostel_registration WHERE regno=? ");
				...
				$stmt->close();
	if($rs) {
		echo"<script>alert('You have already booked a hostel.');</script>";
	} else {
		$roomno=$_POST['room'];
		$feespm=$_POST['fpm'];
		...
		$query="insert into hostel_registration(roomno, feespm, foodstatus, stayfrom, duration, course_code, regno, egycontactno, address) values(?,?,?,?,?,?,?,?,?)";
		$stmt = $mysqli->prepare($query);
		$rc=$stmt->bind_param('iiisissis',$roomno,$feespm,$foodstatus,$stayfrom,$duration,$course,$regno,$emcntno,$caddress);
		$stmt->execute();
...
{
?>
<option value="<?php echo $row->course_code;?>"><?php echo $row->course_code;?>&nbsp;&nbsp;(<?php echo $row->course_fn;?>)</option>
<?php } ?>
</select> </div>
</div>
...
<?php	
$aid=$_SESSION['id'];
	$ret="select * from student where regNo=?";
